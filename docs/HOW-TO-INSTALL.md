...
```bash
helm install <release-name> oci://ghcr.io/dkadetov/strimzi-kafka-connect --version 1.2.0 --values <customized-values-file> --namespace strimzi
```
...
```bash
helm pull oci://ghcr.io/dkadetov/strimzi-kafka-connect --version 1.2.0
```

To preview generated sample manifests, you can use [values-test.yaml](/helm/values-test.yaml) for rendering.

```bash
helm template strimzi-kafka-connect-1.2.0.tgz --values values-test.yaml
```

Create a configuration file with the necessary settings and check the rendered output.

```bash
helm template strimzi-kafka-connect-1.2.0.tgz --values values-custom.yaml
```

Install the chart (for example, in the `strimzi` namespace).

```bash
helm upgrade <release-name> strimzi-kafka-connect-1.2.0.tgz --values values-custom.yaml --install --namespace strimzi
```
